...
At a high-level, we use `redirect` when we want to send a GET request and hit an entirely new action. However, using `redirect` is not always necessary where you could simply use `render`. You should not make the browser need to make a fresh call unless you really have to. Always question when you are using `redirect_to` and if it is the right thing, or perhaps a `render` would be better.
